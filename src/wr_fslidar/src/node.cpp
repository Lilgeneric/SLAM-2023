    ...
    int align_cnt)
{
    ...
    double subset_start_index;
    double subset_end_index;
    double subset_start_angle, subset_end_angle;
    // ros::param::get("~subset_start_angle", subset_start_angle);
    // ros::param::get("~subset_end_angle", subset_end_angle);
    subset_start_angle = - M_PI_2;
    subset_end_angle = M_PI_2;
    ...
    ROS_INFO("msg.angle_max = %f",msg.angle_max);
    ...
    ROS_INFO("ranges[index] = %f \n",msg.ranges[index]);

    angle += msg.angle_increment;
    ROS_INFO("abgle = %f   fmod(angle, M_PI) = %f ",angle, fmod(angle, M_PI));
    ROS_INFO("%d",fmod(angle, M_PI) < 2.0 && fmod(angle, M_PI) > 1.0);

    subset_start_index = (subset_start_angle - msg.angle_min) / msg.angle_increment;
    subset_end_index = (subset_end_angle - msg.angle_min) / msg.angle_increment;

    sensor_msgs::LaserScan subset_scan = msg;
    subset_scan.angle_min = subset_start_angle;
    subset_scan.angle_min = subset_end_angle;
    
    subset_scan.ranges.resize(subset_end_index - subset_start_index + 1);
    for (int i = subset_start_index; i <= subset_end_index; ++i) {
        subset_scan.ranges[i - subset_start_index] = msg.ranges[i];
    }

    // if(fmod(angle, pai) < 2.0 && fmod(angle, pai) > 1.6 )
    pub.publish(subset_scan);
